...
Compiler.prototype = {
  ...
  compile: function() {
    ...
    var js = `const ${Parent} = ${this.extends};\n`;
    js += escodegen.generate(this.ast);
    js = js.replace('extends React.Component', `extends ${Parent}`);
  ...
  visit: function(node) {
    this.depth++;
    if(!this['visit' + node.type]) throw `Unknown node type: ${node.type}`;
  ...
  visitExtends: function(ext) {
    ...
    this.extends = `require('${ext.file.path}').default`;
  },
